...
exports.shareAchievement = async (req, res) => {
    try {
        ...
        res.json({
            ...
            message: `I just unlocked ${achievement.name} on Predyqt — ${instruction(achievement, row.threshold).toLowerCase()}.`,
            level: row.level,
        });
    } catch (err) {
 ...
 *
 * Without it the card reads "A Predyqt member", which is a complete card — the badge is the
    ...
    try {
        ...
        res.json({
            board: board.map((row, i) => ({
                rank: i + 1,
                name: row.displayName || 'Predyqt member',
                avatar: row.avatar,
                points: row.points,
    ...
    try {
        ...
        if (update.optedIn === true && !(update.displayName ?? '').trim()) {
            ...
            if (!existing?.displayName) {
                ...
                update.displayName = (user?.firstName || 'Predyqt member').slice(0, 40);
            }
        }
...
exports.getStats = async (req, res) => {
    try {
        ...
        res.json({
            ...
            note:
                'These are counts of what you have done in Predyqt. Your health results are '
